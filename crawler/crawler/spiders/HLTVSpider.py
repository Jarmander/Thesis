...
class HLTVSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = ['https://www.hltv.org/events/archive?eventType=MAJOR']
        for url in urls:
    ...
    def parse(self, response, **kwargs):
        ...
        for event in event_list:
            if event.rsplit('/')[2].isnumeric():
                ...
                yield scrapy.Request(url=url, callback=self.parse_event, cb_kwargs=metadata)
    ...
    def parse_event(self, response, metadata):
        ...
        for match in group_match_links:
            ...
            yield scrapy.Request(url=url, callback=self.parse_match, cb_kwargs=metadata)
